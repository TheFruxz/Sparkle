 ...
 * @since 1.0
 */
class MoltenCoreAppRuntime(override val identity: String, override val version: Version = 1.0.version) : de.moltenKt.core.application.configuration.MoltenCoreApp(identity, version) {
	 ...
	 */
	internal fun init() {
		de.moltenKt.core.application.configuration.MoltenCoreAppConfigController.apply {
	...
	val appFolder: File by lazy {
		return@lazy de.moltenKt.core.application.configuration.MoltenCoreAppConfigController.getApp(this)?.appFileFolderPath.let { folderPath ->
			if (folderPath != null) {
				return@let folderPath.pathAsFileFromRuntime()
			} else {
				return@let MoltenCoreAppConfigModule.autoGenerateFromApp(this).let {
					de.moltenKt.core.application.configuration.MoltenCoreAppConfigController.addApp(it)
