...
data class BranchContent<T>(
    val key: Key,
    private val tabGenerator: BranchExecutionContext.() -> List<String>, // TODO maybe set or collection is better?
    private val contentGenerator: BranchExecutionContext.() -> T?,
    private val displayGenerator: () -> String = { key.value() },
) : Keyed {

    override fun key() = this.key

    fun generateTab(executionContext: BranchExecutionContext): List<String> = tabGenerator(executionContext)

    fun generateContent(executionContext: BranchExecutionContext): T? = contentGenerator(executionContext)
