...
internal class PointInterchange : StructuredInterchange("point", protectedAccess = true, userRestriction = ONLY_PLAYERS, structure = buildInterchangeStructure {
	...
	branch {
		...
		fun displayServices(executor: InterchangeExecutor, page: Int) {
			val pageValue = SparkleData.savedPoints.content.points.page(page, Constants.ENTRIES_PER_PAGE)
		...
		branch {
			addContent(CompletionAsset.pageCompletion { ceilToInt(SparkleData.savedPoints.content.points.size.toDouble() / Constants.ENTRIES_PER_PAGE) })
